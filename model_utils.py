...
def draw_label(im, label, points):
    ...
    if label in keys:
        cv2.rectangle(
            im, (x, y - dim[1] - baseline), (x + dim[0], y), color_green, cv2.FILLED
        )
        ...
        cv2.rectangle(
            im, (x, y - dim[1] - baseline), (x + dim[0], y), color_red, cv2.FILLED
        )
...
def recognize_plate_ocr(img, coords, reader, region_threshold=0.2):
    ...
    text, score, filter_time = filter_text(
        region=nplate, ocr_result=ocr_result, region_threshold=region_threshold
    )
...
class YOLO_MODEL:
    def __init__(self, weights, device: Optional[str] = None):
        self.model = torch.hub.load(
            "./yolov5", "custom", source="local", path=weights, force_reload=False
        )  # True yerine False
...
def det(img, ocr, model):
    ...
    for detection_as_xyxy in detections_as_xyxy:
        ...
        ocr_res, score, ocr_time, filter_time, clean_time = recognize_plate_ocr(
            img, points, ocr, 0.2
        )
...
class VideoProcessor:
    ...
    def process_frame(self, frame):
        ...
        for detection_as_xyxy in detections_as_xyxy:
            ...
            points = tuple(map(tuple, points))
            ocr_res, score, ocr_time, filter_time, clean_time = recognize_plate_ocr(
                frame, points, self.ocr, region_threshold=0.2
            )
