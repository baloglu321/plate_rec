...
def is_valid_keyword(keyword):
    ...
    return bool(
        re.match(r"^[0-9]{2}[A-Z]{1,3}[0-9]{2,5}$", keyword.replace(" ", "").upper())
    )
...
def validate_keywords(keywords):
    valid_keywords = [
        kw.replace(" ", "").upper() for kw in keywords if is_valid_keyword(kw)
    ]
    invalid_keywords = [
        kw.replace(" ", "").upper() for kw in keywords if not is_valid_keyword(kw)
    ]
...
if invalid_keywords:
    st.error(
        f"Geçersiz tagler: {', '.join(invalid_keywords)}. Lütfen yalnızca plakayı girin (Örneğin : 01ABC234)."
    )
